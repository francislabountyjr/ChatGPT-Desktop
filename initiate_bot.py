...
from PyQt5.QtWidgets import QApplication, QAbstractItemView, QShortcut, QMainWindow, QDialog, QVBoxLayout, QTextEdit, QLineEdit, QPushButton, QLabel, QWidget, QCheckBox, QListWidget, QListWidgetItem, QSizePolicy, QInputDialog, QHBoxLayout, QComboBox, QMessageBox
from PyQt5.QtGui import QPixmap, QMouseEvent, QKeySequence, QImage
from PyQt5.QtCore import Qt, pyqtSignal, QThread, QSize
from PyQt5.QtGui import QKeySequence
from pydub import AudioSegment

from language_module import ChatBot
from voice_module import Text2Audio
from tools_module import WebSearch, WolframAlpha
...
class FloatingIcon(QMainWindow):
    def __init__(self, chat_config=None, text2audio_api_key=None, text2audio_voice='Jarvis', wolfram_app_id=None, parent=None):
        super().__init__(parent)
        self.initUI()
        ...
        self.chat_dialog = ChatDialog(text2audio=Text2Audio(api_key=text2audio_api_key) if text2audio_api_key != None else None, text2audio_voice='Jarvis' if text2audio_voice is None else text2audio_voice, chatbot=ChatBot(config=chat_config), wolfram_app_id=wolfram_app_id)
        self.chat_width = 800
...
class MessageItem(QWidget):
    def __init__(self, message, conversation_id, parent_id, is_user=True, parent=None):
        ...
        if self.is_user:
            label.setStyleSheet("background-color: #9b30ff; border-radius: 10px; padding: 10px; height: auto; float: right;")
        else:
            label.setStyleSheet("background-color: #4d94ff; border-radius: 10px; padding: 10px; height: auto; float: left;")
...
class ManageConversationsDialog(QDialog):
    ...
    def populate_conversations_list(self, refresh=False):
        if refresh or self.conversations is None:
            self.conversations = self.chatbot.get_conversations()
    ...
    def delete_conversation(self):
        ...
        self.chatbot.delete_conversation(conversation_id=conversation['id'])
    ...
    def rename_conversation(self):
        ...
        if ok:
            ...
            self.chatbot.change_title(title=new_name, conversation_id=conversation['id'])
...
class ChatDialog(QDialog):
    def __init__(self, text2audio=None, text2audio_voice='Jarvis', chatbot=None, device="cuda:0" if torch.cuda.is_available() else "cpu", audio2text="openai/whisper-large-v2", wolfram_app_id=None, database=None):
        super().__init__()
...
Summaries: SUMMARIES*"""
        ...
        self.pm = PromptManager(database=database)
        self.prompt = None
    ...
    def initUI(self):
        ...
        self.char_count_label = QLabel("0/4000")
    ...
    def summarize_pdf(self, url, target_words=1000, target_words_per_page=500, num_attempts=3, render=False):
        ...
        for i, text in enumerate(texts):
            try:
                ...
                if render:
                    ...
                    self.add_message("Bot: Loading...", is_user=False)
                    QApplication.processEvents()
                response_ = self.chatbot.get_response(prompt, conversation_id=conversation_id, parent_id=parent_id)
                for partial_response in response_:
                    if render:
                        # Remove the previous message
                        self.conversations_list.takeItem(self.conversations_list.count()-1)
                        self.add_message("Bot: " + partial_response, is_user=False)
                        QApplication.processEvents()
                    response = partial_response
                summaries.append(response.strip())
                conversation_id = self.chatbot.conversation_id
                parent_id = self.chatbot.parent_id
            except:
                time.sleep(3)
                prompt = self.pm.replace_prompt_variables(self.summarize_text, {
                    'current_page': i+1,
        ...
        self.chatbot.conversation_id = original_conversation_id
        self.chatbot.parent_id = original_parent_id
        return summary
    ...
    def format_text(self, text_=None):
        ...
        for query_ in web_search_and_summarize_queries:
            if self.format_cache.get("{web_search_and_summarize(query=" + query_ + ")}") is not None:
                ...
                conversation_id = self.conversation_id
                parent_id = self.parent_id
                web_search_and_summarize = self.chatbot.get_response(prompt=self.format_web_search_prompt(web_search, query=query, citations=citations), conversation_id=None, parent_id=None)
                response = ''
                for response_ in web_search_and_summarize:
                    response = response_
                self.chatbot.change_title(title=f'{query} Summarization')
                self.chatbot.reset()
                self.chatbot.conversation_id = conversation_id
                self.chatbot.parent_id = parent_id
    ...
    def conversationManager(self):
        dialog = ManageConversationsDialog(chatbot = self.chatbot)
        result = dialog.exec_()
    ...
    def reset_conversation_list(self, conversation_id):
        ...
        if conversation_id is not None:
            conversation = self.chatbot.get_conversation(conversation_id=conversation_id)
            for message in list(conversation['mapping'].values()):
                if message['message'] != None:
                    id = message['id']
                    parent_id = message['parent']
                    message = message['message']
                    content = ''.join(message['content']['parts'])
                    is_user = message['author']['role'] == 'user'
                    if is_user:
    ...
    def sendMessage(self):
        ...
        chat_response = self.chatbot.get_response(prompt=message)
        for partial_response in chat_response:
            # Remove the previous message
            self.conversations_list.takeItem(self.conversations_list.count()-1)
            self.add_message("Bot: " + partial_response, is_user=False)
            QApplication.processEvents()
        chat_response = partial_response.strip()


        # Get the conversation ID and parent message ID
        self.parent_id = self.chatbot.parent_id
        ...
        self.add_message("Bot: " + chat_response, is_user=False, conversation_id=self.conversation_id, parent_id=self.parent_id)
...
if __name__ == '__main__':
    try:
        ...
        assert config['chat_config']['access_token'] != '' or config['chat_config']['access_token'] != None, 'Please provide a valid access token in the config.json file'
        app = QApplication(sys.argv)
        floating_icon = FloatingIcon(chat_config=config['chat_config'], text2audio_api_key=config['text2audio_api_key'], text2audio_voice=config['text2audio_voice'], wolfram_app_id=config['wolfram_app_id'])
        floating_icon.show()
        sys.exit(app.exec_())
    except:
        traceback.print_exc()
